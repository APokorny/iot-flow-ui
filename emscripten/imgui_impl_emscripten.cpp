...
bool test_shader(GLuint handle, char const* desc)
{
    ...
    if (status == GL_FALSE) std::cerr << "ERROR: emscripten::Renderer::create_programs : failed to compiler " << desc << std::endl;
    if (log_length > 0)
    {
        std::vector<char> buf;
...
bool test_program(GLuint handle, char const* desc)
{
    ...
    if (status == GL_FALSE) std::cerr << "ERROR: emscripten::Renderer::create_programs : failed to link " << desc << std::endl;
    if (log_length > 0)
    {
        std::vector<char> buf;
        buf.resize(log_length + 1);
...
struct RestoreState
{
    T value;
    F fun;
    RestoreState(GLenum state, F&& f) : value{}, fun{std::move(f)} { glGetIntegerv(state, static_cast<GLint*>(&value)); }
...
struct RestoreState<T[N], F>
{
    T value[N];
    F fun;
    RestoreState(GLenum state, F&& f) : value{}, fun{std::move(f)} { glGetIntegerv(state, value); }
    ~RestoreState() { fun(value); }
};
template <typename T, typename F>
...
void emscripten::Renderer::create_font_texture()
{
    // Build texture atlas
    ...
    auto last_texture = restore<GLint>(GL_TEXTURE_BINDING_2D, [](GLint tex) { glBindTexture(GL_TEXTURE_2D, tex); });
...
void emscripten::Renderer::render_imgui_data(ImDrawData& draw_data)
{
    ImGuiIO& io = ImGui::GetIO();
    int fb_width = static_cast<int>(draw_data.DisplaySize.x * io.DisplayFramebufferScale.x);
    int fb_height = static_cast<int>(draw_data.DisplaySize.y * io.DisplayFramebufferScale.y);
    ...
    auto restore_last_sampler = restore<GLint>(GL_SAMPLER_BINDING, [](GLint last_sampler) { glBindSampler(0, last_sampler); });
#endif
    ...
    auto restore_poly =
        restore<GLint[2]>(GL_POLYGON_MODE, [](GLint[2] lm) { glPolygonMode(GL_FRONT_AND_BACK, static_cast<GLenum>(lm[0])); });
#endif
    auto restore_view_port = restore<GLint[4]>(GL_VIEWPORT, [](GLint vp[4]) { glViewport(vp[0], vp[1], vp[2], vp[3]); });
    auto restore_scissor = restore<GLint[4]>(GL_SCISSOR_BOX, [](GLint sc[4]) { glViewport(sc[0], sc[1], sc[2], sc[3]); });
...
emscripten::Renderer::Renderer(int glsl_version) : glsl_version{glsl_version}
{
...
void emscripten::Renderer::setup_renderer()
{
    create_programs();
    create_font_texture();
}

void emscripten::Renderer::create_programs()
{
    auto restore_vertex_buffer = restore<GLint>(GL_VERTEX_ARRAY_BINDING, [](GLint vertex_array) { glBindVertexArray(vertex_array); });
    auto restore_array_buffer =
        restore<GLint>(GL_ARRAY_BUFFER_BINDING, [](GLint array_buffer) { glBindBuffer(GL_ARRAY_BUFFER, array_buffer); });
    auto restore_texture = restore<GLint>(GL_TEXTURE_BINDING_2D, [](GLint texture) { glBindTexture(GL_TEXTURE_2D, texture); });
    ...
    // Create shaders
    const GLchar* vertex_shader_with_version[2] = {glsl_version_text.c_str(), vertex_shader};
    vert_handle = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(vert_handle, 2, vertex_shader_with_version, NULL);
    glCompileShader(vert_handle);
    test_shader(vert_handle, "vertex shader");

    const GLchar* fragment_shader_with_version[2] = {glsl_version_text.c_str(), fragment_shader};
...
void emscripten::Renderer::finish_frame() { emscripten_webgl_commit_frame(); }

void emscripten::Renderer::cleanup_font_texture()
{
    if (font_texture)
    {
        auto& io = ImGui::GetIO();
...
void emscripten::Renderer::cleanup_programs()
{
    if (vbo_handle) glDeleteBuffers(1, &vbo_handle);
    if (element_handle) glDeleteBuffers(1, &element_handle);
    vbo_handle = element_handle = 0;

    if (shader_handle && vert_handle) glDetachShader(shader_handle, vert_handle);
    if (vert_handle) glDeleteShader(vert_handle);
    vert_handle = 0;

    if (shader_handle && frag_handle) glDetachShader(shader_handle, frag_handle);
...
emscripten::SystemIntegration::SystemIntegration() : create_ui{[]() {}}
{
    ...
    emscripten_set_scroll_callback_on_thread(target, data, true, [](int type, EmscriptenUiEvent const* event, void*) -> int { return 0; },
                                             this_thread);
    emscripten_set_blur_callback_on_thread(target, data, true, [](int type, EmscriptenFocusEvent const* event, void*) -> int { return 0; },
                                           this_thread);
    emscripten_set_focus_callback_on_thread(target, data, true, [](int type, EmscriptenFocusEvent const* event, void*) -> int { return 0; },
                                            this_thread);
    emscripten_set_focusin_callback_on_thread(target, data, true,
                                              [](int type, EmscriptenFocusEvent const* event, void*) -> int { return 0; }, this_thread);
